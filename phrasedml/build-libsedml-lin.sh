...
pwd
rm -rf *
cmake -DCMAKE_CXX_FLAGS="-L/whlbldr/install/libnuml/lib -L/whlbldr/install/libsbml-experimental/lib" -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=$ROOT/install/libsedml -DLIBSBML_LIBRARY=$LIBSBML -DLIBSBML_INCLUDE_DIR=$LIBSBML_INCLUDE -DLIBSBML_STATIC=ON -DWITH_PYTHON=OFF -DLIBNUML_LIBRARY=$NUML -DLIBNUML_INCLUDE_DIR=$NUML_INCLUDE -DLIBNUML_STATIC=ON $ROOT/src/libsedml
