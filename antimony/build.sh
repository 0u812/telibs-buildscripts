...
pwd
rm -rf *
"$CMAKE" "$CMAKE_GEN" $CMAKE_PLATFORM_FLAGS -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=$ROOT/install/$INSTNAME -DWITH_SBML=ON -DWITH_STATIC_SBML=ON -DWITH_LIBSBML_LIBXML=ON -DWITH_PYTHON=ON -DLIBSBML_INSTALL_DIR=$LIBSBML_INSTALL_DIR -DWITH_LIBSBML_COMPRESSION=ON -DPYTHON_EXECUTABLE=$PYTHON -DPYTHON_INCLUDE_DIR=$PYTHON_INCLUDE $PYTHON_LINKING -DWITH_QTANTIMONY=OFF -DPYTHON_LOCAL_INSTALL=ON -DPYTHON_SYSTEM_INSTALL=OFF -DWITH_CONDA_BUILDER=OFF -DWITH_CELLML=ON -DCELLML_API_INSTALL_DIR=$CELLAPI -DBZIP_LIBRARY=$LIBBZIP2 -DICONV_LIBRARY=$LIBICONV -DLIBXML_LIBRARY=$LIBXML2 -DZDLL_LIBRARY=$ZLIB -DEXTRA_LIBS=$ANTIMONY_EXTRA_LIBS -DSWIG_EXECUTABLE=$SWIG -DCMAKE_CXX_FLAGS="-std=c++11" $ROOT/src/$SRCNAME
